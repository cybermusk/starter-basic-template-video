      ...
      {!hasSubMenu && (
        <li className='group relative whitespace-nowrap'>
          <Link
            href={link?.href}
            target={link?.target}
            className={`ud-menu-scroll mx-8 flex py-2 text-base font-medium text-dark hover:text-[#cda780] dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 ${router.route === '/' ? 'lg:text-white' : ''}`}>
            ...
            {link.subMenus.map((sLink, index) => {
              return (
                <Link
                  ...
                  href={sLink.href}
                  target={link?.target}
                  className='block rounded px-4 py-[10px] text-sm text-body-color hover:text-[#cda780] dark:text-dark-6 dark:hover:text-[#cda780]'>
                  {/* 子菜单SubMenuItem */}
